...
import com.gb.cwsup.scan.camera.CameraManager;
import com.gb.cwsup.scan.decoding.CaptureActivityHandler;
import com.gb.cwsup.scan.decoding.InactivityTimer;
...
public class MipcaActivityCapture extends BaseActivity implements Callback {
	...
	private boolean vibrate,light=false;
...
//		});
	...
	private void setopenclick() {
		if (camera==null) {
			camera = Camera.open();  
		}
		mParameters = camera.getParameters();
		final Button lightbutton = (Button) findViewById(R.id.viewfinder_light);
		lightbutton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if (light) {
					...
					camera.startPreview();
					light=true;
					lightbutton.setBackgroundResource(R.drawable.shoudian_white128);
				}else {
					mParameters.setFlashMode(Camera.Parameters.FLASH_MODE_OFF);//打开Camera.Parameters.FLASH_MODE_OFF则为关闭  
					camera.setParameters(mParameters);
					camera.stopPreview();
					light=false;
					lightbutton.setBackgroundResource(R.drawable.shoudian_hui128);
				}
			}
		});
